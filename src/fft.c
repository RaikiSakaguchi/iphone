...
// samples 配列に対して FFT→IFFT を実行
void fft_filter(sample_t *samples, long n) {
  if (!pow2check(n)) die("fft_filter: n が 2 の累乗でない");
  complex double *X = malloc(sizeof(complex double) * n);
  complex double *Y = malloc(sizeof(complex double) * n);
  if (!X || !Y) die("malloc");  // X が NULL か、Y が NULL か

  sample_to_complex(samples, X, n);
  fft(X, Y, n);
  // ここで Y に対してスペクトル処理が可能
  ifft(Y, X, n);
  complex_to_sample(X, samples, n);
